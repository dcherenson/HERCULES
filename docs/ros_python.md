# How to use HERCULES with Robot Operating System (ROS)

HERCULES and ROS can be integrated using Python. Some example ROS node are provided demonstrating how to publish data from HERCULES as ROS topics.

## Prerequisites

These instructions are for Ubuntu 20.04, ROS Noetic, UE 5.X+ and latest HERCULES release.
You should have these components installed and working before proceeding.

**Note that you need to install the Python module first for this to work. More information [here](apis.md) in the section 'Installing HERCULES Package'.**
...
## Setup workspace and HERCULES package

#### Option A: Create a new ROS package in your catkin workspace following these instructions.  

[Create a new ROS package](http://wiki.ros.org/ROS/Tutorials/CreatingPackage) called HERCULES or whatever you like.
...
## Build ROS HERCULES package

Change directory to your top level catkin workspace folder i.e. ```cd ~/catkin_ws```  and run ```catkin_make```
This will build the HERCULES package.  Next, run ```source devel/setup.bash``` so ROS can find the new package.
You can add this command to your _~/.bashrc_ to load your catkin workspace automatically.

## How to run ROS HERCULES nodes

First make sure you are running a HERCULES project and that the simulation is playing.

The implemented HERCULES node can be run using ```rosrun airsimros airsim_publish.py```.
