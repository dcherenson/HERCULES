classdef AirSimClient < handle
    ...
    methods(Static)        
        ...
        function carControls = getCarControls()
            carClient = py.hercules_cosysairsim.CarClient();
            carClient.confirmConnection();
            carControls = py.hercules_cosysairsim.CarControls();
        end

        function droneClient = getDroneControls()
            droneClient = py.hercules_cosysairsim.MultirotorClient();
    ...
    methods
        ...
        function [image, timestamp] = getCameraImage(obj, sensorName, cameraType, vehicleName, annotationLayer)
            ...
            if cameraType == 1 || cameraType == 2 || cameraType == 3 || cameraType == 4
                image_request = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraType), true, false);
            elseif cameraType == 10
                image_request = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraType), false, false, annotationLayer);
            else
                image_request = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraType), false, false);
            end
            ...
            image_response = py.hercules_cosysairsim.ImageResponse();
        ...
        function [images, timestamp] = getCameraImages(obj, sensorName, cameraTypes, vehicleName, annotationLayers)
            ...
            for i = 1: numel(cameraTypes)
                if cameraTypes(i) == 1 || cameraTypes(i) == 2 || cameraTypes(i) == 3 || cameraTypes(i) == 4
                    image_requests{i} = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraTypes(i)), true, false);
                elseif cameraTypes(i) == 10
                    image_requests{i} = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraTypes(i)), false, false, annotationLayers(i));
                else
                    image_requests{i} = py.hercules_cosysairsim.ImageRequest(sensorName, int32(cameraTypes(i)), false, false);
                end
            end
        ...
        function [intrinsics, sensorPose] = getCameraInfo(obj, sensorName, vehicleName)
            ...
            tempRequest = py.hercules_cosysairsim.ImageRequest(sensorName, int32(1), true, false);
            ...
            testImageResponse = py.hercules_cosysairsim.ImageResponse();
