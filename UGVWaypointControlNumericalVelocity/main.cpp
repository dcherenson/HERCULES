//NOTE!!! The controller here gets its velocity from the numerical differentiation of pose instead of speed directly
// to allow usage of different physics engines in the same environment where timestepping is not cross-compatible
// among the physics engines. 
// This script loads a list of waypoints from a text file and
// commands the UGV to sequentially pass through them using the
// CarRpcLibClient's moveOnPathNumericalVelocity function.
//
...
int main(int argc, char *argv[])
{
    ...
    // Parameters for moveOnPathNumericalVelocity
    float timeout_sec = 1e6f; // max allowed time (s)
    float lookahead   = 4.0f; // lookahead distance (m)

    // Execute using the numerical‐velocity variant
    bool success = client.moveOnPathNumericalVelocity(
        waypoints,
        desired_velocity,
        timeout_sec,
        lookahead,
