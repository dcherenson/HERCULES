...
// Commented out by SSG, original function below causes seg fault on UE5
...
bool FObjectFilter::matchesComponent(UActorComponent *actor_component) const
{
    // Function below modified by SSG so that object detection works with crowd character BP type also
    // ——— Original flags ———
    bool bMatchesStaticMesh = !static_mesh_;
    bool bMatchesSkeletalMesh = !skeletal_mesh_;
    bool bMatchesWildcardMeshName = wildcard_mesh_names_.Num() == 0;
    ...
    // ——— Static‐mesh branch (only if a static‐mesh match or wildcard is requested) ———
    if (static_mesh_ || wildcard_mesh_names_.Num() != 0)
    {
        if (UStaticMeshComponent *StaticMeshComponent = Cast<UStaticMeshComponent>(actor_component))
        {
            UStaticMesh *mesh = StaticMeshComponent->GetStaticMesh();
            // explicit exact‐mesh check
            if (static_mesh_ && mesh == static_mesh_)
            {
                bMatchesStaticMesh = true;
            }
            // wildcard‐by‐name check, but only if mesh pointer is valid
            if (wildcard_mesh_names_.Num() != 0 && mesh && mesh->IsValidLowLevel())
            {
                const FString &name = mesh->GetName();
                if (isMatchAnyWildcard(name))
                {
                    bMatchesWildcardMeshName = true;
                }
            }
    ...
    // ——— Skeletal‐mesh branch (only if a skeletal‐mesh match or wildcard is requested) ———
    if (skeletal_mesh_ || wildcard_mesh_names_.Num() != 0)
    {
        if (USkeletalMeshComponent *SkeletalMeshComponent = Cast<USkeletalMeshComponent>(actor_component))
        {
            USkeletalMesh *skel = SkeletalMeshComponent->GetSkeletalMeshAsset();
            // explicit exact‐mesh check
            if (skeletal_mesh_ && skel == skeletal_mesh_)
            {
                bMatchesSkeletalMesh = true;
            }
            // wildcard‐by‐name check, only if asset pointer is valid
            if (wildcard_mesh_names_.Num() != 0 && skel && skel->IsValidLowLevel())
            {
    ...
    if (actor_class_)
    {
        if (AActor *owner = actor_component->GetOwner();
            owner && owner->GetClass()->IsChildOf(actor_class_))
        {
            bMatchesActorClass = true;
        }
    }
    ...
    {
        if (AActor *owner = actor_component->GetOwner();
            owner && owner->ActorHasTag(actor_tag_))
        {
            bMatchesActorTag = true;
        }
    }

    // ——— Return (duplicate bMatchesComponentClass must be kept) ———
    return bMatchesStaticMesh &&
           bMatchesSkeletalMesh &&
           bMatchesWildcardMeshName &&
           bMatchesActorClass &&
           bMatchesActorTag &&
           bMatchesComponentClass &&
           bMatchesComponentClass &&
           bMatchesComponentTag;
}
