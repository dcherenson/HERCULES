...
The original planning baseline was `codex/ros2` at
`9003d21e30c9628152581e84d21ee986573e092e` (`integrate distributed ROS 2 target
tracking`), inspected 2026-09-12. The implementation is now present in the
current branch snapshot `0d72264b2ed8c3f1f39f1d781e4683e10c482832`
(`codex/ros2`, 2026-09-15). This document remains the behavioral contract and
acceptance plan for future fixes/reproduction; its original “Add” lists are
reconciled with the files that actually exist below. Paths are relative to the
repository root.

The implementation target is **behavioral parity with the executable Python
mission at the planning baseline**. There are material differences between the
requested mathematical description, Python execution, and the ROS actuator
boundary. These are resolved explicitly below rather than silently corrected.
Conformal prediction, cooperative localization, tracking mathematics,
formation redesign, and Vulkan/Metal rendering repair are excluded.

## Current implementation status

| Area | Current tree | Status boundary |
|---|---|---|
| Pure CBF core | `ros2/src/hercules_cbf` | Present; ROS-independent C++17/Eigen with native OSQP C API, Python-compatible clipping/projection/fallbacks, and focused GTest/parity coverage. |
| ROS adapter/observers | `ros2/src/hercules_cbf_ros` | Present; conversion, obstacle cache, read-only perception/collision observers, and tests. |
| Interfaces/configuration | `hercules_interfaces/msg/*` and `hercules_mission_ros/config/rural_tracking_{no_cbf,mestres,wang}.yaml` | Present and wired into the mission launch. |
| Reproduction | `docker/ros2/reproduce_cbf.sh`, `ros2/validation/cbf/compare_cbf_modes.py` | Present; defaults to dry-run and 30 seconds, and still needs a reachable simulator for live state/origin discovery. |
| Mac live camera parity | Native UE 5.2.1 Metal + Docker Desktop | Not accepted yet; validate editor/plugin architecture, RPC host/network, image encoding, and capture-pose/origin behavior on the Mac. |

The current implementation intentionally consolidates the native OSQP wrapper in
`ros2/src/hercules_cbf/src/filter.cpp`; the planned `osqp_solver.hpp/.cpp` split
was not created. Likewise, the actual focused core test is `test_cbf.cpp` and
the actual parity fixture is generated by `cbf_parity_dump.cpp`; the planned
`test_constraints.cpp`, `test_filter.cpp`, and `fixtures/cbf_cases.json` are not
present. Treat the actual tree and fresh test output as authoritative.
...
| Tracking transport | `ros2/src/hercules_tracking_ros`: per-agent trackers, epoch transport, neighbor graph, and a Python target-camera observer wrapping existing Python code. CBF obstacle/collision transport is separate in `ros2/src/hercules_cbf_ros`. |
...
| Validation/media | `ros2/validation/rural_nominal/render_validation.py`, `compare_tracking_modes.py`, `ros2/validation/cbf/compare_cbf_modes.py`, `docker/ros2/reproduce.sh`, `docker/ros2/reproduce_cbf.sh`, and Python `modules/mission_plots.py`. CBF logs are compared by mode; exact request replay remains the numerical parity gate. |

Verified during planning:

- The original planning probe used Docker image `hercules-ros2:humble` (local image ID `1f050821da94`) with Ubuntu 22.04.5, CMake 3.22.1, Eigen 3.4.0, Python OSQP 1.0.4, NumPy 1.21.5, and SciPy 1.8.0; it did not contain scikit-learn. The checked-out `herculesvenv` then had NumPy 1.26.4, SciPy 1.15.3, scikit-learn 1.7.1, and OSQP 1.0.4. These are **historical planning observations**, not the current Mac image state.
- The historical planning run reported **134 Python distributed-mission tests** and **115 ROS tests** passing. The later CBF report records a named phase-3 image with **135 Python tests**, **129 ROS test results**, and **2 CBF metrics tests**. None of these counts is a substitute for a clean run at the current `HEAD`; record the image digest and architecture with every acceptance run.
...
- No live simulator mission was run during the original planning probe. The separate `ros2/validation/cbf/REPORT.md` and `COMPARISON_500_REPORT.md` are historical Linux/replay records; they do not establish current macOS/Metal behavior or normalized safety performance.
...
## Phase 1 — pure C++ CBF core (implemented; re-run acceptance on each host)

**Deliverable:** a simulator-independent `ros2/src/hercules_cbf` C++17/Eigen
library with literal Python OSQP-path behavior and directly compared fixtures.
No ROS headers/runtime, AirSim, RPC, mission, or tracking dependency. The
current implementation meets the package boundary; the clean standalone and
Mac Docker acceptance gates still need to be run from the current checkout.

### Files and dependencies

The initial implementation landed these files:
...
ros2/src/hercules_cbf/test/test_cbf.cpp
ros2/src/hercules_cbf/README.md
docker/ros2/install_osqp.sh
```

The list is intentionally not a promise that the omitted planned split files
exist: OSQP setup/solve is in `src/filter.cpp`, and the parity cases are emitted
by `cbf_parity_dump.cpp` and checked by the Python harness. Add finer-grained
fixtures/tests only as a follow-up when they preserve the same contract.
...
Checkpoint: the isolated core/dependency/test work is present in the current
branch. Keep rollback at the mission flags (`cbf_enabled=false`,
`cbf_obstacle_source=none`) or revert the isolated implementation commit; debug
from a single serialized request/constraint dump, not a live mission.

## Phase 2 — ROS and live mission integration (implemented; re-run acceptance on each host)
...
The planned `cbf_adapter_parity_dump.cpp`,
`test_python_cpp_coupling_parity.py`, and `test_cbf_mission_transport.py` are
not present in the current tree. Their intended coupling checks are partially
covered by the adapter/cache GTests and the live JSONL diagnostics; do not cite
them as executed tests until dedicated files are added.
...
## Phase 3 — reproduce target-tracking + CBF experiments (implemented through replay; live camera remains open)

**Deliverable:** explicit no-CBF/Mestres/Wang run configurations, reusable Python
obstacle perception, matched experiment logs, metrics and extensions to the
existing animation. The current tree contains the configuration/observer/replay
pieces and a historical Linux report. Deterministic request/replay evidence is
not equivalent to a normalized live safety experiment; Mac/Metal camera parity
...
The current tree does not contain every file in the original design sketch:
there is no `cbf_adapter_parity_dump.cpp`,
`test_python_cpp_coupling_parity.py`, `test_cbf_mission_transport.py`, or
`fixtures/cbf_cases.json`. Keep those as future coverage, not as implicit
acceptance results. The actual core dump/harness is listed in Phase 1 and the
actual CBF mode driver is `docker/ros2/reproduce_cbf.sh`.
...
The current launch loads `mission_config`/`cbf_config` parameter files and
exposes the mode switches below. Keep numerical mission defaults in
`ruralTargetTrackingConfig()` unchanged and log the fully resolved values plus
any override. Do not create decorative unused configurations. Configure these
shared values for all three matched runs:
...
Current mode-driver commands (the simulator must already be reachable for
state/origin discovery):

```bash
./docker/ros2/reproduce_cbf.sh --mode no_cbf --obstacles none --observation truth --dry-run
...
The driver resolves one of the checked-in mode files, passes explicit launch
values, records mode metadata/HEAD and artifacts, and starts the mission. It
currently defaults to `dry_run=true` and the launch's 30-second duration; use
`--live` (or `--dry-run false`) for actuation, and invoke `ros2 launch` directly
with `duration_sec:=...` when another duration is required. It does not itself
make a Python and ROS trajectory identical. Truth-observation seed 7 remains
the current default. Each run gets a separate directory under the ignored
`ros2/validation/cbf/artifacts/` tree.

### macOS/Metal acceptance boundary

On macOS, keep Unreal native and ROS in Docker Desktop. Install/use UE 5.2.1
for macOS, verify the editor and AirSim plugin architecture (the Darwin build
defaults to native `arm64`; `HERCULES_MAC_ARCH=x86_64` is an explicit legacy
escape hatch), and launch without Linux NVIDIA/Vulkan variables.
macOS camera validation uses Metal and must inspect the received image encoding;
`is_vulkan` is an RGB/BGR encoding switch, not a renderer selector. The Compose
file's `network_mode: host` is the Linux assumption. If Docker Desktop host
networking is unavailable, use `host.docker.internal` consistently for every
AirSim client and record that bridge path, including the direct pose bridge,
wrapper, reset/benchmark helpers, Python observer, and video recorder. A TCP
port probe or deterministic C++ parity pass is not evidence that Metal capture
poses, image encoding, or obstacle geometry match the Linux reference.
...
The original six-unit breakdown below describes how the work was intended to be
implemented. At the current snapshot, the Phase 1/2 packages and Phase 3
configuration/replay scaffolding are already in the tree; Luna should begin by
re-running their acceptance gates and resolving portability/coverage gaps, not
by assuming every originally sketched file exists. The remaining high-risk
boundary is a fresh rendered macOS/Metal perception run with one consistent
Docker Desktop RPC path.
