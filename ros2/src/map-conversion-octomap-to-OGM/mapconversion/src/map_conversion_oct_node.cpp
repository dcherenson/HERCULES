...
public:
  MapToMap() : Node("map_conversion")
  {
    ...
    // Create a timer to continuously publish map messages (adjust period as needed)
    timer_ = this->create_wall_timer(
        100ms, std::bind(&MapToMap::pub, this));
  }
  ...
  {
    // Set resolution and initialize MapConverter if needed
    if (MC == nullptr)
    {
    ...
    update2Dmap(minMax);
    // Also publish immediately on receiving new data
    pub();
  }
  ...
  // Update 2D map in the region of the axis-aligned bounding box (aabb)
  ...
  void pub()
  {
    // Ensure MC has been initialized (i.e. that we have received at least one octomap message)
    if (MC == nullptr)
      return;
    ...
    // Publish RViz visualization for floor height map
    ...
    // Publish RViz visualization for ceiling height map
    ...
    // Publish height map
    if (pubHeightMap->get_subscription_count() != 0 || pub_qos_transient_local)
    {
      for (int y = 0; y < MC->map.sizeY(); y++)
      {
        for (int x = 0; x < MC->map.sizeX(); x++)
    ...
    // Publish RViz visualization for slope map
