...
def main(raw_data_dir, bag_out):
    ...
    for t in all_ts:
        ...
        if t in imu_data:
            ...
            imu = Imu()
            imu.header.stamp = Time(sec=ts_nsec // 1_000_000_000,
        ...
        for sub, topic in img_topics.items():
            if t in img_map[sub]:
                ...
                img_msg.header.stamp = imu.header.stamp
                img_msg.header.frame_id = "camera_link"

                writer.write(f"/{topic}", serialize_message(img_msg), ts_nsec)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Usage: pack_to_ros2bag.py RAW_DIR BAG_OUT")
        sys.exit(1)
